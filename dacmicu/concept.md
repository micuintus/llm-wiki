...
6. ~~**Subagents** (`@pi-dacmicu/subagent`)~~ — **dropped 2026-05-08, refined 2026-05-08 evening 2.** Standing on the existing Pi subagent ecosystem. **v1 simplification: depend only on `tintinweb/pi-subagents`** (which exposes Claude Code-idiomatic `Task`/`get_subagent_result`/`steer_subagent` tools — LLM training-known shapes, free prompt tokens). No multi-mode `delegate()` API; the LLM uses tintinweb's `Task` tool when it wants a subagent. Variant A (inline) is the default; Variant B (subagent) is opt-in via tintinweb's tools. `HazAT/pi-interactive-subagents` integration deferred to v1.x when evolve usage data justifies the added complexity. See [research-2026-05-08-evening2](research-2026-05-08-evening2-simplification.md) and [ecosystem/subagents](../ecosystem/subagents.md).

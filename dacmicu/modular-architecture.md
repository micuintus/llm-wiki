...
|---|---|---|---|---|
| 1 | `@pi-dacmicu/base` | ~150 | none | Deterministic in-session loop primitive: `agent_end` driver, `signal_loop_success` tool, compaction preservation, abort detection, state rehydration. Exports the runtime as a library for consumers. |
...
| todo widget (status) | `ctx.ui.setStatus(key, text)` (footer/status bar) | `tmustier/pi-extensions/pi-ralph-wiggum/index.ts:206` ("🔄 name (iter/max)") |
...
| todo widget (above-editor, reactive) | `ctx.ui.setWidget(key, (tui, theme) => Component & {dispose?()}, opts)` — factory form, `render(width):string[]` + `invalidate():void` | **`davebcn87/pi-autoresearch/extensions/pi-autoresearch/index.ts:1294,1316,1348`** — the production reference; collapsed/expanded states; user-configurable shortcut keys via `/extensions/pi-autoresearch.json` |
...
- **Visibility & navigability** are inherited from Hopsken — ConversationViewer (`src/ui/conversation-viewer.ts`, 243 LOC) is the gold-standard opencode-Tab-switch analog. Agent-tree widget (`src/ui/agent-widget.ts`, 488 LOC) provides always-visible state.
...
| 2. Persistent status | `ctx.ui.setWidget("todo", factory)` (component factory form, **not** the static `string[]` form) | Reactive, width-aware. Collapsed (one-liner: "TODO: 3/7 done — buy milk") and expanded (full list) states, toggle via configurable shortcut. Match `pi-autoresearch/extensions/pi-autoresearch/index.ts:1294-1380`. |
...
Key idiomatic detail from pi-autoresearch: `setWidget` factory must return `{ render(width: number): string[]; invalidate(): void }`. `render` is invoked on every paint, so any state mutation reflects immediately without manual repaint plumbing. Collapse/expand is a flag in extension state (`dashboardExpanded`) that selects between two factory branches.
